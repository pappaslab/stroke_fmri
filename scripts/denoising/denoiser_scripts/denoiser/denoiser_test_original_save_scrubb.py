def denoise(img_file, tsv_file, out_path, col_names=None, hp_filter=None, lp_filter=None, out_figure_path=None,
    ...
    if hp_filter:
        ...
        df = make_first_level_design_matrix(frame_times, period_cut=period_cutoff, add_regs=df.to_numpy(),
    ...
    if hp_filter:
        ...
        model_first = regression.OLSModel(df[hp_cols].to_numpy())
        ...
        model_second = regression.OLSModel(df[nr_cols].to_numpy())
    ...
    try: 
        ...
        y = df_orig[FD_name].to_numpy()
	
        Nremove = []
        sc_idx = []
        for thr_idx, thr in enumerate(FD_thr):
            idx = y >= thr
            #path_save_scrub=out_figure_path+'/fd_ind.npy'
            #np.save(path_save_scrub,idx)
            #pdb.set_trace()
        ...
        path_save_scrub=out_figure_path+'/fd_ind.npy'
        np.save(path_save_scrub,sc_idx)
        #pdb.set_trace()
        Nplots = len(FD_thr)
